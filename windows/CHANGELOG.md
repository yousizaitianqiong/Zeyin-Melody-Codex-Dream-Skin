...
- 修复 Codex Desktop 26.803 更新 composer DOM 后，首页与普通任务页输入框失去主题表面和描边的问题。共享运行时现优先识别 data-codex-composer-root 下的稳定 surface/footer 语义锚点，并继续公开 composer / composer-toolbar；旧类仅作回退，搜索框、弹窗与审批卡片不会被误标。新版 composer 保留原生尺寸和项目栏分离布局，首页引用文字移到输入根节点上方并在窄窗或矮窗隐藏；普通任务页只由通用桥接解除原生 important 边框锁定，最终颜色与悬停状态仍由已验证的主题 Safe CSS 控制，合同能力不变。
...
- 同步 v1.5.12 版本号，发布 Codex 26.803 composer 语义桥接与原生布局兼容修复。
